...
filename = 'T100_370002_CAM1.avi'
pathname = os.environ['HOME'] + '/videos_ondometro2/'
...
while(cap.isOpened()):
    ...
    if frame is None:
        ...
        #img1 = img[952:1003,18:320,:]
        # im = np.copy(im1[954-100:998+100,50-30:308+100,:])
        im1 = np.copy(im[952:1003,18:320,:])
        ...
        #time_ocr_str = []
        time_ocr_str = pytesseract.image_to_string(im1)
        # stop
        ...
        ctrss, hier = cv2.findContours(im_th.copy(),
        ...
        # for rect in rects:
            ...
            roi_hog_fd1 = hog(roi, orientations=9, pixels_per_cell=(14, 14), cells_per_block=(1, 1))
	...
	#cv2.imwrite(os.environ['HOME'] + '/BRUNO/videos_ondometro/frames/%s_%s_frame_%s.bmp' %(filename[:-4], time_ocr_str[-1], f), im1)
        #cv2.imwrite(os.environ['HOME'] + '/videos_ondometro/frames/ii_20160316_%s_%s.bmp' %(b, str(f).zfill(5)), im1)
        cv2.imwrite(os.environ['HOME'] + '/videos_ondometro2/frames/%s_is_20160316_%s.bmp' %(str(f).zfill(5), b), im1)
        #cv2.imwrite(os.environ['HOME'] + '/videos_ondometro/frames/%s_is_20160316_%s.bmp' %(str(f).zfill(5), a), im1)        
        #cv2.imwrite(os.environ['HOME'] + '/videos_ondometro/frames/%s_%s_frame_%s.bmp' %(filename[:-4], time_ocr_str[-1], f), im1)
        # stop
