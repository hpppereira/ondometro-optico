...
def plot_creasts(frame_c1, frame_c2, cristas_c1, cristas_c2,
                 dist_hor_cr_c1, dist_hor_cr_c2, alt_cr, nframe, idx2):
    ...
    plt.savefig(pathout + 'img_{}.png'.format(nframe), bbox_inches='tight')
    plt.close('all')
    return
...
if __name__ == '__main__':
    ...
    video_c1 = os.path.join(os.environ['HOME'] +
    '/Documents/ondometro_videos/laboceano/CAM3/T100/', 'T100_570003_CAM3.avi')

    # pathname video superior
    video_c2 = os.path.join(os.environ['HOME'] +
    '/Documents/ondometro_videos/laboceano/CAM1/T100/', 'T100_570003_CAM1.avi')

    # path de saida das imagens
    pathout = os.environ['HOME'] + '/Documents/ondometro_results/teste_01/'

    # numero do frame inicial e final do trecho do video a processar
    timei, timef = '00:52', '01:10'
    ...
    for nframe in np.arange(nfi_c1, nff_c1):
        ...
        if diff_dist_creasts > 0.5:
            ...
            plot_creasts(frame_c1, frame_c2, cristas_c1, cristas_c2,
                         dcr1, dcr2, alt_cr, nframe, idx2)

    alturas = pd.DataFrame(alturas)
    alturas.to_csv('out/alturas.csv')
